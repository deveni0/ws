...
var MsgUrlOptionsSchema = zod.z.object({
  img: zod.z.union([zod.z.string().url(), zod.z.instanceof(Buffer)]).optional(),
  title: zod.z.string().optional(),
  body: zod.z.string().optional(),
  mentions: zod.z.array(zod.z.string()).optional(),
  big: zod.z.boolean().optional()
});
...
var COMMAND_PROPS = {
  ...
  restrictions: ["owner", "group", "private", "botAdmin"],
...
var CommandSystem = class {
  ...
  async executeCommand(cmd, msg, context) {
    ...
    for (const [failed, type, extra] of checks) {
      if (failed) {
        await handleAccessControl(this.bot, msg, type, extra);
        return;
      }
    }
    if (cmd.cooldown) {
      ...
      if (lastUsed) {
        ...
        if (remaining > 0) {
          await handleAccessControl(this.bot, msg, "cooldown", remaining);
          return;
        }
      }
    }
    ...
    try {
      ...
      await handleAccessControl(this.bot, msg, "error", error);
    }
  }
      ...
      if (commands) {
        ...
        for (const cmd of cmdArray) {
          if (typeof cmd === "function") {
            ...
            logger_default.info(`[${(/* @__PURE__ */ new Date()).toISOString()}] before handler (${fileName})`);
  ...
  unloadFile(filePath) {
    ...
    if (!commandNames?.length) {
      this.commands = this.commands.filter((cmd) => {
        ...
        return !cmdPath || path.resolve(cmdPath) !== normalizedPath;
      });
    } else {
    ...
    this.beforeHandlers = this.beforeHandlers.filter((handler) => {
      ...
      return !handlerPath || path.resolve(handlerPath) !== normalizedPath;
    });
...
zod.z.object({
  ...
  reply: zod.z.custom(),
  react: zod.z.custom(),
  lid2jid: zod.z.custom().optional(),
  jid2lid: zod.z.custom().optional(),
  delete: zod.z.custom(),
...
var group_event = (OOP) => {
  OOP.sock.ev.on("group-participants.update", async (data) => {
    try {
      ...
      const jid = metadata.participants.find((x) => x.id == data.author)?.phoneNumber;
      const ppUrl = await OOP.sock.profilePictureUrl(jid, "image");
      const event = {
        id: data.id,
        chat: data.id,
        ...
        action: data.action,
        author: jid,
        url: ppUrl || "https://files.catbox.moe/hm9iq4.jpg",
